...
#define VIDEO_TASK_STACK_SIZE           (12 * 1024)
...
typedef struct {
    ...
    /** Stable storage for xTaskCreate arg: must NOT be caller stack (use-after-return → EBADF on DQBUF). */
    int stream_video_fd;
...
static inline esp_err_t video_stream_stop(int video_fd)
{
    ...
    if (ioctl(video_fd, VIDIOC_STREAMOFF, &type)) {
        ESP_LOGW(TAG, "VIDIOC_STREAMOFF failed errno=%d (fd may already be invalid)", errno);
    }
...
static void video_stream_task(void *arg)
{
    ...
    while (1) {
        ...
        if (res != 0) {
            ...
            if (e == EBADF) {
                ESP_LOGE(TAG, "VIDIOC_DQBUF EBADF: bad video fd (check task arg is not stack)");
                vTaskDelay(pdMS_TO_TICKS(50));
                continue;
            }
            ESP_LOGD(TAG, "VIDIOC_DQBUF errno=%d, retry", e);
...
esp_err_t app_video_stream_task_start(int video_fd, int core_id)
{
    ...
    if (video_stream_start(video_fd) != ESP_OK) {
        ESP_LOGE(TAG, "video_stream_start failed");
        return ESP_FAIL;
    }

    app_camera_video.stream_video_fd = video_fd;
    BaseType_t result = xTaskCreatePinnedToCore(
        video_stream_task,
        "video stream task",
        VIDEO_TASK_STACK_SIZE,
        &app_camera_video.stream_video_fd,
        VIDEO_TASK_PRIORITY,
...
void app_video_get_resolution(uint32_t *w, uint32_t *h)
{
    if (w) {
        *w = app_camera_video.camera_buf_hes;
    }
    if (h) {
        *h = app_camera_video.camera_buf_ves;
    }
}
