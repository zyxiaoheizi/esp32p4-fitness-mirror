...
MFN::MFN(const char *model_name)
{
    ...
    m_image_preprocessor = new dl::image::FeatImagePreprocessor(
        m_model, {127.5, 127.5, 127.5}, {127.5, 127.5, 127.5}, true);
    m_postprocessor = new dl::feat::FeatPostprocessor(m_model);
    auto &outs = m_model->get_outputs();
    ESP_ERROR_CHECK(outs.empty() ? ESP_ERR_NOT_FOUND : ESP_OK);
    m_feat_len = outs.begin()->second->get_size();
}
...
uint16_t HumanFaceRecognizer::get_last_feat_id()
{
    return m_db.get_last_feat_id();
}
